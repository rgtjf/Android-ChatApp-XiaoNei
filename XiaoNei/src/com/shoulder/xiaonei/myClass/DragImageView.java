...
public class DragImageView extends ImageView {
	...
	void setScale(float scale) {
		...
		if (scale > 1 && this.getWidth() <= MAX_W) {
			current_Left = (int)(this.getLeft() - disX * times);
			current_Top = (int)(this.getTop() - disY * times) ;
			current_Right = (int)(this.getRight() + disX * times);
			current_Bottom = (int)(this.getBottom() + disY * times);
		...
		// 缩小
			...
			current_Left = this.getLeft() + 1*disX;
			current_Top = this.getTop() + 1*disY;
			current_Right = this.getRight() - 1*disX;
			current_Bottom = this.getBottom() - 1*disY;
			/***
			 * 在这里要进行缩放处理
			 */
			// 上边越界
